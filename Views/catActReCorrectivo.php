        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="true">
          ...
          <li class="nav-item menu-close">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="./catActRePreventivo.php" class="nav-link ">
                  <i class="far fa-circle nav-icon"></i>
        ...
        <div class="col">
          ...
          <div class="card text">
            <div class="card-header">
              <h2>Actividad Realizada a Impresora (Correctivo)</h2> 
            </div>
              ...
              <form id="marcaForm" class="row g-3 needs-validation" style="border-radius: 20px;" novalidate>
                    ...
                    <div class="form-group col-6" >
                        <label for=""> ID</label>
                        <input type="" class="form-control" id="txtIdActvidadc" name="" disabled>
                      
                    </div>

                  <div class="form-group col-6">
                        <label for=""> Actividad</label>
                        <input type="" class="form-control" id="txtActividadc" >
                      
                    </div>

                    <button  class="btn btn-outline-success  btn-sm btn-block" type="button" id = "btActividadc"><b>Agregar</b></button>
            

<div class="card-body">
    <table id="tablaActividadc" class="table table-bordered table-striped">
        <thead>
            <tr>
                <th>Codigo</th>
                <th>Actividad</th>
        ...
        <tbody>
            <?php while ($columna = mysqli_fetch_assoc($resultadosactc)) { ?>
                <tr>
                    <td><?php echo $columna['id_actividadc']; ?></td>
                    <td><?php echo $columna['actividadc_actividadesc']; ?></td>
                    <td>
                        <?php if ($columna['status_actividadc'] == '0') { ?>
                            Inactivo
                        <?php } else { ?>
                            Activo
                        <?php } ?>
                    ...
                    <td>
                        <?php if ($columna['status_actividadc'] == '0') { ?>
                            <button class="btn btn-success btn-sm" onclick="cambiarEstadoActividadc(<?php echo $columna['id_actividadc']; ?>, '1')">Activar</button>
                        <?php } else { ?>
                            <button class="btn btn-danger btn-sm" onclick="cambiarEstadoActividadc(<?php echo $columna['id_actividadc']; ?>, '0')">Desactivar</button>
                        <?php } ?>
                    </td>
                </tr>
            <?php } ?>
...
<script src="../Assests/dist/js/formActividadC.js"></script>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

<script>
function cambiarEstadoActividadc(id_actividadc, nuevo_status) {
    $.ajax({
        url: '../Controller/updateStatusActividadc.php',
        type: 'POST',
        data: {
            id_actividadc: id_actividadc,
            statusactc: nuevo_status
        },
        dataType: 'json',
