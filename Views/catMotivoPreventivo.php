        ...
        <div class="col">
          ...
          <div class="card text">
            <div class="card-header">
              <h2>Motivo del Ingreso de la Impresora (Preventivo)</h2> 
            </div>
              ...
              <form id="marcaForm" class="row g-3 needs-validation" style="border-radius: 20px;" novalidate>
                    ...
                    <div class="form-group col-6" >
                        <label for=""> ID</label>
                        <input type="" class="form-control" id="txtIdMotivop" name="" disabled>
                      
                    </div>

                  <div class="form-group col-6">
                        <label for=""> Motivo</label>
                        <input type="" class="form-control" id="txtMotivop" >
                      
                    </div>

                    <button  class="btn btn-outline-success  btn-sm btn-block" type="button" id = "btMotivop"><b>Agregar</b></button>
            

<div class="card-body">
    <table id="tablaMotivop" class="table table-bordered table-striped">
        <thead>
            <tr>
                <th>Codigo</th>
                <th>Motivo</th>
        ...
        <tbody>
            <?php while ($columna = mysqli_fetch_assoc($resultadosm)) { ?>
                <tr>
                    <td><?php echo $columna['id_motivop']; ?></td>
                    <td><?php echo $columna['motivosp_motivop']; ?></td>
                    <td>
                        <?php if ($columna['status_motivop'] == '0') { ?>
                            Inactivo
                        <?php } else { ?>
                            Activo
                        <?php } ?>
                    ...
                    <td>
                        <?php if ($columna['status_motivop'] == '0') { ?>
                            <button class="btn btn-success btn-sm" onclick="cambiarEstadoMotivop(<?php echo $columna['id_motivop']; ?>, '1')">Activar</button>
                        <?php } else { ?>
                            <button class="btn btn-danger btn-sm" onclick="cambiarEstadoMotivop(<?php echo $columna['id_motivop']; ?>, '0')">Desactivar</button>
                        <?php } ?>
                    </td>
                </tr>
            <?php } ?>
...
<script src="../Assests/dist/js/formMotivoP.js"></script>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

<script>
function cambiarEstadoMotivop(id_motivop, nuevo_status) {
    $.ajax({
        url: '../Controller/updateStatusMotivop.php',
        type: 'POST',
        data: {
            id_motivop: id_motivop,
            statusm: nuevo_status
        },
        dataType: 'json',
