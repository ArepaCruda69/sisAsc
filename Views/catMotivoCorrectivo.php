        ...
        <div class="col">
          ...
          <div class="card text">
            <div class="card-header">
              <h2>Motivo del Ingreso de la Impresora (Correctivo)</h2> 
            </div>
              ...
              <form id="marcaForm" class="row g-3 needs-validation" style="border-radius: 20px;" novalidate>
                    ...
                    <div class="form-group col-6" >
                        <label for=""> ID</label>
                        <input type="" class="form-control" id="txtIdMotivoc" name="" disabled>
                      
                    </div>

                  <div class="form-group col-6">
                        <label for=""> Motivo</label>
                        <input type="" class="form-control" id="txtMotivoc" >
                      
                    </div>

                    <button  class="btn btn-outline-success  btn-sm btn-block" type="button" id = "btMotivoc"><b>Agregar</b></button>
            

<div class="card-body">
    <table id="tablaMotivoc" class="table table-bordered table-striped">
        <thead>
            <tr>
                <th>Codigo</th>
                <th>Motivo</th>
        ...
        <tbody>
            <?php while ($columna = mysqli_fetch_assoc($resultadosc)) { ?>
                <tr>
                    <td><?php echo $columna['id_motivoc']; ?></td>
                    <td><?php echo $columna['motivosc_motivoc']; ?></td>
                    <td>
                        <?php if ($columna['status_motivoc'] == '0') { ?>
                            Inactivo
                        <?php } else { ?>
                            Activo
                        <?php } ?>
                    ...
                    <td>
                        <?php if ($columna['status_motivoc'] == '0') { ?>
                            <button class="btn btn-success btn-sm" onclick="cambiarEstadoMotivoc(<?php echo $columna['id_motivoc']; ?>, '1')">Activar</button>
                        <?php } else { ?>
                            <button class="btn btn-danger btn-sm" onclick="cambiarEstadoMotivoc(<?php echo $columna['id_motivoc']; ?>, '0')">Desactivar</button>
                        <?php } ?>
                    </td>
                </tr>
            <?php } ?>
...
<script src="../Assests/dist/js/formMotivoC.js"></script>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>

<script>
function cambiarEstadoMotivoc(id_motivoc, nuevo_status) {
    $.ajax({
        url: '../Controller/updateStatusMotivoc.php',
        type: 'POST',
        data: {
            id_motivoc: id_motivoc,
            statusc: nuevo_status
        },
        dataType: 'json',
